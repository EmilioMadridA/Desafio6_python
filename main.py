...
while correcto and n_pregunta < 3*int(p_level):
    ...
    if continuar == 'y':
        ...
        # Informar al usuario si acerto y preguntar si desea continuar
        if correcto and n_pregunta < 3*int(p_level):
            ...
            os.system(op_sys)
        # Informar al usuario cuando todas las preguntas se hayan respondido correctamente    
            ...
            time.sleep(3)
            os.system(op_sys)
        # Informar al usuario de su error. Cerrar app.
        else: 
            print(f'Lo siento, conseguiste {n_pregunta - 1} respuestas correctas,\n Sigue participando!!')
            time.sleep(3)
            exit()
    # En caso de no continuar, cerrar app
    else: 
